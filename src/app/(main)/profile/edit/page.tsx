...
export default function EditProfilePage() {
  ...
  if (!session) {
    return <div className="flex items-center justify-center py-20"><p className="text-[#636E72]">התחבר כדי לערוך את הפרופיל.</p></div>;
  }
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      <h1 className="mb-2 text-[32px] font-bold tracking-[-0.01em] text-[#2D3436]">עריכת פרופיל</h1>
      <p className="mb-8 text-[14px] text-[#636E72]">עדכן את הפרטים האישיים שלך</p>

      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="rounded-[16px] border border-[#E8ECF1] bg-white p-6 space-y-5">
          <div>
            <label className="mb-2 block text-[13px] font-semibold text-[#636E72]">שם מלא</label>
            <input value={name} onChange={(e) => setName(e.target.value)} required className={inputClass} />
          </div>
          <div>
            <label className="mb-2 block text-[13px] font-semibold text-[#636E72]">עיר</label>
            <input value={city} onChange={(e) => setCity(e.target.value)} placeholder="תל אביב" className={inputClass} />
          </div>
          <div>
            <label className="mb-2 block text-[13px] font-semibold text-[#636E72]">טלפון</label>
            <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="050-0000000" className={inputClass} />
          </div>
          <div>
            <label className="mb-2 block text-[13px] font-semibold text-[#636E72]">ביו</label>
            <textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={4} placeholder="ספר על עצמך..." className={inputClass} />
          </div>
          <div>
            <label className="mb-2 block text-[13px] font-semibold text-[#636E72]">קישור לתמונת פרופיל</label>
