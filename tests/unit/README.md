...
Also covers chat rules, gateway signing, rate limits, password policy, seller slot checks, daddy onboarding readiness (`seller-ready.test.ts`: checklist completeness and searchable-seller filters), nearby-request matching (`request-match.test.ts`), the notification feed, disputes, order status updates, the admin moderation queue, and the 1–10 review contract (`review-ratings.test.ts`, `review-route.test.ts`).
