...
test.describe("Real job loop", () => {
  ...
  test("buyer request, seller quote, accept, start, deliver, complete, 1–10 review", async ({ browser }) => {
    ...
    try {
      ...
      await expect(seller.page.getByText("החלפתי את הברז, הישן בשקית").first()).toBeVisible();

      await buyer.page.reload();
      await expect(buyer.page.getByAltText("תמונת המסירה 1").first()).toBeVisible({ timeout: 10000 });
      await expect(buyer.page.getByText("החלפתי את הברז, הישן בשקית").first()).toBeVisible();
