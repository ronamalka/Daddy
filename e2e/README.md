...
| `job-loop.spec.ts` | Real marketplace loop in Hebrew: login, post a request, seller quote, buyer accept → local job, seller start → deliver, buyer complete → 1–10 review; plus instant-book a slot from the seller price list (the catalog-unification path; `/gigs/:id` checkout is not the public catalog anymore) |
